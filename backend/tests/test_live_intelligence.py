...
def test_similarity_and_verified_valuation() -> None:
    from decimal import Decimal
    from app.services.comparables import valuation
    subject = Property(name="Subject", address="1 Main", city="Hudson", state="NY", square_feet=2000, acreage=2, bedrooms=3, bathrooms=2, year_built=2000)
    comparable = ComparableProperty(address="2 Main", sale_price=Decimal("400000"), square_feet=2000, acreage=2, bedrooms=3, bathrooms=2, year_built=2000, distance_miles=1, price_per_square_foot=Decimal("200"), verification_status="verified", source="Approved feed", confidence=0.9)
    report = valuation(subject, [comparable])
    assert report["estimated_market_value"] == 400000
    assert report["confidence"] > 0


def test_unavailable_comparables_do_not_produce_valuation() -> None:
    from app.services.comparables import valuation
    subject = Property(name="Subject", address="1 Main", city="Hudson", state="NY")
    report = valuation(subject, [])
    assert report["estimated_market_value"] is None
